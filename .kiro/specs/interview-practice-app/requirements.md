...
### 1.1 目的と独自性
2025年開校の愛知県公立中高一貫校（明和高校附属中、刈谷高校附属中、津島高校附属中、半田高校附属中）の入試二次面接試験（個人面接15分）に特化した練習アプリを開発する。

**核心的差別化要素**：
- **実際の面接完全再現**: 二次面接合格者の実際の面接フローを完全再現
- **段階的深堀り対話システム**: 一つの回答に対する5-6層の自然な深堀り質問
- **小学6年生向け特化**: 既存AI面接アプリ（steach、SpeakViz等）は大学生・社会人向け
- **志願理由書完全連動**: 個別の志願理由書と教育理念の融合による完全パーソナライズ
- **保護者参加設計**: 小学生の学習をサポートする保護者向け機能群
...
#### 2.1.2 実際の面接フロー再現セッション（4段階進行）
- **時間管理**: 15分タイマー + 段階別時間配分表示
- **進行制御**: 実際の面接フローに基づく4段階自動進行
- **対話的深堀り**: AI面接官による自然な会話フロー再現
